...
#include "./scenes/abstract_sculpture.inl"
#include "./scenes/geomatric_art.inl"
#include "./scenes/geometric_tower.inl"
#include "./scenes/mirror_room_with_tower.inl"
...
#include "Scene.h"
#include "Sphere.h"
#include "./scenes/mirror_room.inl"
...
void RayTracer::init(int scene_id) {
    ...
    if (scene_id == 1)
        ...
        scene = std::unique_ptr<Scene> (mirror_room());
    else if(scene_id == 5)
        scene = std::unique_ptr<Scene> (geometric_tower());
    else if(scene_id == 6)
        scene = std::unique_ptr<Scene> (mirror_room_with_tower());
...
Ray RayTracer::ray_thru_pixel(int i, int j) {
    ...
    float alpha = (2.0f * (i + x) / camera.width - 1.0f) * camera.aspect * tan(camera.fovy / 2.0f);
    float beta = (1.0f - 2.0f * (j + y) / camera.height) * tan(camera.fovy / 2.0f);
